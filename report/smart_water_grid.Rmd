...
Note that in this report we denote the measured quantity with 'Variable'.
...
We decided not to focus on the 'other' variable for now, especially since we have limited background of their meaning.
...
## Challenges, methods and assumptions

Some challenges

* Data set: measuring frequency differs per data set, blocks of missing data, string values in numeric variables
* Lot of data, many response variables, many potential predictors
* Potential time lags in correlation

Potential solutions

* Subsetting by selecting variables and time range
* Aggregated statistics per hour: looking at mean, variance, max and a binary statistic that determines whether max > threshold
...
We try to find relationships between measured quantities by looking at correlations of their aggregated statistics. For example: Since we are interested in changes (deltas), one approach would be to correlate Eventlab hourly variance with hourly variance in flow measurements.
